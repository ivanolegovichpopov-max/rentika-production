...
class RentalHistoryEntry(BaseModel):
    ...
    id добавлен в 49-м проходе (обратная связь по карточке аренды —
    "нужен механизм корректировки сумм") — раньше запись журнала была
    read-only и ни на что не ссылалась; теперь по id записи "Записан
    платёж" можно оформить POST .../history/{entry_id}/correct, который
    сам найдёт исходную сумму и посчитает разницу (см. докстринг
    correct_rental_payment ниже)."""
...
class RentalPaymentCorrection(BaseModel):
    """Исправление опечатки в уже записанном платеже (49-й проход,
    обратная связь: "нужен механизм корректировки сумм, вдруг опечатался").

    Раньше единственным способом исправить сумму было записать ВТОРОЙ
    платёж с отрицательным amount, вручную посчитав разницу — человеку
    приходилось делать арифметику в уме, а в журнале оставались две никак
    не связанные записи без объяснения, что это была за компенсация.
    Здесь человек вводит не разницу, а то, какой должна была быть сумма
    ИСХОДНОГО платежа — corrected_to. Сервер сам находит исходную запись по
    entry_id и её ДЕЙСТВУЮЩЕЕ значение (исходная сумма плюс все более ранние
    исправления именно этой записи, если платёж уже правили раньше — см.
    correct_rental_payment), считает delta от него и применяет к
    rental.paid_amount тем же способом, что и обычный платёж. Технически
    это по-прежнему добавление новой записи в журнал (action=
    "payment_correction"), а не переписывание/удаление старой — история
    платежей остаётся честной и ничего не стирает, как и положено
    финансовому журналу, просто с явной пометкой "что на что исправлено"
    и ссылкой на исходную запись."""
