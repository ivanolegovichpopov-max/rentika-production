...
class EmployeeOut(BaseModel):
    ...
    # Включена ли 2FA у аккаунта сотрудника (доп. проход после 67-го —
    # "подсветка сотрудников без 2FA при обязательном требовании
    # должности") — та же видимость, что email/phone/last_login_at: None
    # для всех, кроме владельца/платформенного админа (см. _employee_out).
    # Нужно фронту, чтобы сопоставить с Position.require_2fa и показать
    # реальный пробел в защите, а не полагаться на то, что сотрудник просто
    # ещё не заходил.
