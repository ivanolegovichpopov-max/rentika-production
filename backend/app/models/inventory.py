...
class ClientNote(Base):
    ...
    настолько строго: с 37-го прохода автор может удалить СВОЮ запись, и
    только в течение короткого окна после добавления (см.
    CLIENT_NOTE_DELETE_WINDOW_MINUTES в app/api/routes/clients.py) — это
    закрывает случай "опечатался, сразу заметил", но не даёт задним числом
    почистить историю недельной/месячной давности. Владелец бизнеса
    (ctx.full_access) может удалить любую запись без ограничения по
    времени — та же модерационная логика, что и у DashboardNote
