...
// Быстрая почта клиенту прямо из строки таблицы/карточки (26-й проход) —
// тем же принципом, что и IconPhone/IconSend выше, только для mailto:.
...
// Прикреплённый документ (карточка клиента, раздел "Документы", 26-й проход)
// — стандартный глиф файла с загнутым уголком.
...
// Дата рождения (фильтр "Дни рождения на этой неделе", форма клиента) —
// стандартный глиф торта/подарка упрощён до календаря-с-звёздочкой не
// потребовался — используем простой подарок, узнаваемый и лёгкий SVG.
...
// 42-й проход — журнал изменений аренды (RentalHistorySection.tsx).
