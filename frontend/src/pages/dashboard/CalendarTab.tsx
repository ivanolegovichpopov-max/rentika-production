                  ...
                  rows.push(
                    <div className="cal-row" key={e.id} style={{ ["--days" as string]: DAYS }}>
                      <div
                        ...
                        // Клик открывает карточку оборудования (55-й проход,
                        // обзор — "как считаешь, должна открываться карточка
                        // оборудования?"). Раньше здесь был функциональный
                        // аналог ("клик фильтрует по категории этой позиции")
                        // — общей деталь-панели для оборудования тогда
                        // действительно нигде не было. Она с тех пор появилась
                        // (EquipmentDetailPanel, EquipmentTab.tsx — клик по
                        // строке таблицы там открывает её), и onOpenEquipment
                        // уже был проброшен в CalendarTab (используется из
                        // открытой карточки аренды) — тот же клик здесь просто
                        // не был на неё заведён. Быстрый фильтр по категории
                        // клика лишился не просто так — для него теперь есть
                        // отдельный выпадающий список с чекбоксами в тулбаре
                        // (см. calCategoryFilter выше), дублировать то же самое
                        // действие двумя способами избыточно.
                        title={`Открыть карточку «${e.name}»` + (e.warehouse ? `, склад: ${e.warehouse}` : "")}
