 ...
 *    CreateRentalModal (52-й проход — живёт в ./CreateRentalModal.tsx,
 *    RentalsTab.tsx его только ре-экспортирует) с предзаполненным клиентом
 *    и позициями текущей аренды.
...
/** Экспортирована (49-й проход) — та же модалка теперь используется и с
 * карточки списка "Аренды" (RentalsTab.tsx) как быстрое действие "Записать
 * оплату" для неоплаченных аренд, не только изнутри панели деталей. */
...
export function RentalDetailPanel({
  ...
  // Открыть модалку быстрого продления (живёт в ./ExtendRentalModal.tsx,
  // 52-й проход — см. докстринг файла выше про "полностью read-only" в
  // части правки аренды).
  ...
  // 52-й проход — каждая в своём файле в этой же папке) не экспортированы
  // отсюда, RentalsTab.tsx лишь получает от родителя сигнал "открой их для
  // этой аренды". Ни одно из действий не закрывает панель — после reload
  // аренд из контекста статус/бейджи в уже открытой панели обновятся сами,
  // тот же принцип, что и у "Продлить".
  ...
  // аренды (CreateRentalModal, 52-й проход — живёт в ./CreateRentalModal.tsx).
  ...
  // возвращённой/отменённой всё уже закрыто. И только когда позиций больше
  // одной (49-й проход, обратная связь: "ломает логику, когда позволяет
  // вернуть раньше остальных единственную вещь") — при одной позиции
  // подсказка "остальное продолжит числиться в аренде" неправда, там нет
  // никакого "остального": отметить единственную позицию и есть закрыть
  ...
  return (
    <div className="slideover">
      ...
      {/* Бейджи и кнопки действий — раньше были одним flex-wrap рядом
          (49-й проход, обратная связь: "при большом числе бейджей 'Принять
          возврат'/'Изменить' смещаются во вторую строку, это неправильно —
          нужно закрепить"). Позиция кнопок зависела от того, сколько
          бейджей поместилось на первой строке и какой они длины — сейчас
          бейджей только два (статус + оплата), но при появлении третьего
          кнопки поехали бы дальше. Разведены на два самостоятельных ряда:
          верхний (бейджи) переносится как угодно, ни на что не влияя;
      ...
      <div className="slideover-section">
        ...
        <div className="kv-grid">
          ...
          {/* Без justify-content: flex-end (49-й проход, обратная связь:
              "равняется по правому краю — выбивается из общего дизайна") —
              во всей "Финансы"/"Стоимость"/"Оплата" ни одно значение нигде
              не прижато к правому краю, все стоят по левому краю своей
              колонки сразу после отступа от подписи (сам kv-grid этого не
              делает, .mono тоже — проверено рендером). flex-end здесь был
              единственным исключением и утаскивал вместе с иконкой саму
              сумму, ломая общее выравнивание. Просто display:flex ради
              gap — по умолчанию это уже flex-start, то есть левый край, как
              у всех остальных строк. */}
