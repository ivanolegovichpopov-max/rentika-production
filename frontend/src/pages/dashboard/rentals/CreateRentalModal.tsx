...
export function CreateRentalModal({
  ...
  return (
    ...
    >
      <div className="field">
        <label>Клиент</label>
        {/* "+ Добавить нового клиента" — в одной строке с Dropdown'ом,
            справа от него (60-й проход: прямое указание — "задача была
            простая: разместить кнопку в одну строку с выпадающим меню,
            справа"). Ссылка — второй элемент фиксированной ширины справа от
            Dropdown'а. Пока открыта панель быстрого добавления
            (quickAddOpen) — ссылку прячем совсем: сама панель открывается
            под этой строкой, там уже есть свои "Отмена"/"Добавить клиента".

            Про цвет ссылки (60-й проход, "кнопка всё ещё большая" уже в
            строчной раскладке): шрифт .link-btn (11.5px) на деле МЕНЬШЕ
            подписи "Клиент" над полем (12px) — проверено вычисленными
            стилями, разница была не в размере текста, а в акцентном синем
            цвете, который рядом с рамкой дропдауна бросался в глаза сильнее
            размера шрифта. Приглушил цвет ссылки до var(--muted).

            Про размер самого Dropdown'а (61-й проход, "кнопка 'Выберите
            клиента' большая, должна быть аккуратная, маленькая, как
            раньше") — см. комментарий у самого Dropdown ниже: style-проп
            flex:1, который растягивал его на всю ширину строки, убран
            совсем. */}
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          {/* Поиск (46-й проход, по итогам обзора формы "Новая аренда" — при
              ...
              Без style-пропа (61-й проход: "кнопка 'Выберите клиента'
              большая, должна быть аккуратная, маленькая, как раньше") —
              БЕЗ style Dropdown остаётся обычным компактным .cat-filter
              (inline-block по контенту закрытой кнопки, тот же вид, что и у
              всех остальных фильтров в приложении); style={{flex:1}}
              раньше явно растягивал его на всю строку — именно это и не
              понравилось, а не размер шрифта/текста внутри. Ссылка
              "+ Добавить нового клиента" остаётся рядом, справа от него,
              просто сама строка больше не тянется на всю ширину поля. */}
          <Dropdown
            value={clientId}
          ...
          {!quickAddOpen && (
            <button
              type="button"
              className="link-btn"
              style={{ flex: "none", whiteSpace: "nowrap", color: "var(--muted)" }}
