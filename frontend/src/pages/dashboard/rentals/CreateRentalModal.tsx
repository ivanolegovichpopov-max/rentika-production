    ...
    >
      <div className="field">
        ...
        {/* "+ Добавить нового клиента" — в одной строке с Dropdown'ом,
            ...
            справа", предыдущие эксперименты с размером/переносом под поле
            были лишними, возвращено к исходной идее 58-го прохода). Dropdown
            занимает всю доступную ширину строки (flex:1 — тот же style-проп
            попутно включает width:100% на внутренней кнопке Dropdown'а, см.
            Dropdown.tsx), ссылка — второй элемент фиксированной ширины
            справа от него. Пока открыта панель быстрого добавления
            (quickAddOpen) — ссылку прячем совсем: сама панель открывается
            под этой строкой, там уже есть свои "Отмена"/"Добавить клиента".

            Про "кнопка всё ещё большая" (жалоба повторилась и в этой,
            строчной, раскладке): по факту шрифт .link-btn (11.5px) даже
            МЕНЬШЕ подписи "Клиент" над полем (12px) — проверено вычисленными
            стилями, разница не в размере текста. Визуально "крупной" её
            делает акцентный синий цвет — рядом с рамкой дропдауна на той же
            строке он бросается в глаза сильнее, чем сам размер шрифта.
            Приглушил цвет до var(--muted) (тот же, что у подписей полей) —
            строка/сторона не меняются, ссылка просто перестаёт визуально
            спорить с основным полем. */}
