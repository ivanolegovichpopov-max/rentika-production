        ...
        <div className="modal-body">
          ...
          {showTiered ? (
            <>
              ...
              <div className="field-hint">
                {/* Текст поправлен в 50-м проходе — прежняя формулировка "первые
                    N дней — по фиксированной цене" не совпадала с реальной
                    формулой (itemCostForDays/financeCalc.ts): пока срок не
                    превысил N дней, действует обычная ставка за сутки, а
                    "Цена за период" — это порог, в который упирается стоимость
                    ТОЛЬКО когда срок аренды превысил N дней. Именно эта
                    путаница в тексте подсказки и привела к тому, что в живых
                    данных нашлась позиция, где 14 дней стоят дешевле, чем
                    заявлено в "Цене за период" — см. пример ниже. */}
                Заполните, если хотите зафиксировать цену при аренде дольше N дней: пока срок не превышает N дней —
                действует обычная ставка за сутки; как только он превысил N дней — цена больше не растёт по суткам, а
                упирается в «Цену за период», и каждый полный или начатый шаг сверх него — по отдельной цене за шаг
                своей длины. Например: дольше 14 дней — 690 ₽, затем 190 ₽ за любую начатую неделю (шаг 7 дней)
                после.{" "}
                <button
                  type="button"
                  className="link-btn"
          ...
          {showCostPreview && showTiered && form.period_days && (() => {
            // Пример на ГРАНИЦЕ периода, а не на произвольных 7/30 днях
            // (50-й проход, по итогам всестороннего обзора вкладки "Аренды" —
            // прежний пример "7 дней ≈ … · 30 дней ≈ …" ни разу не задевал
            // сам переход через period_days, поэтому реальный скачок цены
            // ровно там, где ступенчатый тариф начинает работать, был не
            // виден при настройке тарифа — обнаруживался только постфактум,
            // на живой аренде). До границы действует обычная посуточная
            // ставка (см. itemCostForDays/rateLabel), после — period_price:
            // если period_price задан заметно дороже, чем periodDays дней по
            // посуточной ставке, переход через границу даёт скачок в разы, а
            // не плавную скидку — это и стоит увидеть здесь, до сохранения.
            const p = Number(form.period_days);
            const atBoundary = previewCost(p);
            const overBoundary = previewCost(p + 1);
            const expectedStep = previewRate * 1.5; // с запасом на округление/шаг >1 суток
            const jumpIsSuspicious = overBoundary - atBoundary > expectedStep && overBoundary - atBoundary > 0;
            return (
              <div className="field-hint">
                <div>
                  Пример: {p} дн. ≈ {money(atBoundary)} · {p + 1} дн. ≈ {money(overBoundary)}
                </div>
                {jumpIsSuspicious && (
                  <div className="text-critical">
                    Один лишний день сверх периода добавляет {money(overBoundary - atBoundary)} ₽ — заметно больше
                    ставки за сутки ({money(previewRate)} ₽). Проверьте «Цену за период»: пока срок аренды не
                    превышает {p} дн., она не применяется — действует обычная ставка за сутки.
                  </div>
                )}
              </div>
            );
